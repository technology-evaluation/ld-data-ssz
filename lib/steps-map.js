...
function createHierarchie (quad) {
  if (quad.predicate.value === 'http://example.org/hierarchie') {
    const quads = []
    const concepts = quad.object.value.split(';')

    concepts.forEach(concept => {
      const member = quad.subject
      const subject = rdf.namedNode('https://ld.stadt-zuerich.ch/statistics/collection/' + (concept.trim().replaceAll(' ', '_')))
      ...
      quads.push(rdf.quad(subject, rdf.namedNode('http://www.w3.org/2004/02/skos/core#member'), member))
    })
